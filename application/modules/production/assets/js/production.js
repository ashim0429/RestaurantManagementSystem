    ...
    success: function (data) {
      ...
      if (obj != null) {
        ...
        $("#product_id_" + sl)
          ...
          .val("")
          .select2();
        alert("Please purchase this Item ,else!!!");
        $("#unit-total_" + sl).val("");
        return false;
      }
    },
...
function addmore(divName) {
  ...
  if (count == limits) {
    ...
    var tab4 = tabindex + 4;
    newdiv.innerHTML =
      '<td class="span3 supplier"><select name="product_id[]" id="product_id_' +
      count +
      '" class="postform resizeselect form-control" onchange="product_list(' +
      count +
      ')">' +
      credit +
      '</select></td><td class="text-right"><input type="text" name="product_quantity[]" tabindex="' +
      tab2 +
      '" required  id="cartoon_' +
      count +
      '" class="form-control text-right store_cal_' +
      count +
      '" onkeyup="calprice(this)"  placeholder="0.00" value="" min="0"/>  </td><td class="text-right"><input type="text" tabindex="' +
      tab2 +
      '" id="price_' +
      count +
      '" class="form-control text-right store_cal_' +
      count +
      '"  placeholder="0.00" value="" min="0" readonly/>  </td><td> <input type="hidden" id="total_discount_1" class="" /> <input type="hidden" id="unit-total_' +
      count +
      '" class="" /><input type="hidden" id="all_discount_1" class="total_discount" /><button class="btn btn-danger red text-right" class="btn btn-danger red" type="button" value="Delete" onclick="deleteRow(this)" tabindex="8">Delete</button></td>';
  ...
  $.ajax({
    ...
    success: function (data) {
      if (ischeck == 1) {
        swal(
          "Enable",
          "Enable This Option to show on Production auto complete",
          "success"
        );
      } else {
        swal(
          "Disable",
          "Make This Field Is Optional On Production auto complete.",
          "warning"
        );
      }
    },
