...
class Order extends MX_Controller
{
	...
	{
		if ($this->permission->method('ordermanage', 'read')->access() == FALSE) {
			$this->session->set_flashdata('exception', display('You do not have permission to accept this item.'));
		}
		...
		$itemids = explode(',', $itemid);
		$varientids = explode(',', $varient);
		$itemidsv = array_map('trim', $itemids);
		$varientidsv = array_map('trim', $varientids);
		$i = 0;
