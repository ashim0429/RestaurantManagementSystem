...
function orderaccept(ordid, kitid) {
  var values = $('input[name="item' + ordid + kitid + '"]:checked')
    .map(function () {
      return $(this).val();
    })
    .get()
    .join(",");
  var varient = $('input[name="item' + ordid + kitid + '"]:checked')
    .map(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data == 1) {
        $('input[name="item' + ordid + kitid + '"]:checked').removeAttr(
          "disabled"
        );
        ...
        $('input[name="item' + ordid + kitid + '"]:checked').attr("alt", 1);
        $('input[name="item' + ordid + kitid + '"]:checked').prop(
          "disabled",
          true
        );
      }
...
function ordercancel(ordid, kitid) {
  ...
  var values = $(
    'input[name="item' + ordid + kitid + '"]:checked:not(:disabled)'
  )
    .map(function () {
      return $(this).val();
    })
...
function onprepare(ordid, kitid) {
  ...
  $.ajax({
    ...
    success: function (data) {
      var numberOfChecked = $(
        'input[name="item' + ordid + kitid + '"]:checkbox:checked'
      ).length;
      var totalCheckboxes = $(
        'input[name="item' + ordid + kitid + '"]:checkbox'
      ).length;
      var delonefromall = totalCheckboxes - 1;
      if (
        delonefromall == numberOfChecked ||
        totalCheckboxes == numberOfChecked
      ) {
